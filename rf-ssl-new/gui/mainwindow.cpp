...
MainWindow::MainWindow(bool start_capture, bool enforce_affinity)
{
  ...
  setWindowTitle("SSL-Vision System, released as GPLv3, Windows version by Piyamate Wasuntapichaikul");
  ...
  //expand capture control
  QList<VarItem*> control = tmodel->findItems("Capture Control");
  for(int i=control.size()-1; i>=0; i--)
    tree_view->expandAndFocus(control[i]->getVarType());
