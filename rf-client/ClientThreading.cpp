...
ViewUpdateThread::ViewUpdateThread ( SoccerScene *_soccerScene, QMutex* _drawMutex )
{
  soccerScene = _soccerScene;
  drawMutex = _drawMutex;
//  log_control = new LogControl();
//  connect(this->soccerScene->playLogfile, SIGNAL(pressed()), this, SLOT(playLogfilePressed()) );
...
//    Log_Frame* log_frame;
    ...
    if ( client.receive ( packet ) && !play)
    {
      ...
      if ( packet.has_detection() )
      {
        ...
        drawMutex->lock();        
        soccerScene->UpdateBalls ( balls,detection.camera_id() );
        //Robot info:
//        soccerView->UpdateRobots ( detection );
      ...
      {
        cout << "packet geometry" << endl;
        drawMutex->lock();
        const SSL_GeometryData & geom = packet.geometry();
        const SSL_GeometryFieldSize & field = geom.field();
        soccerScene->LoadFieldGeometry ( ( SSL_GeometryFieldSize& ) field );
        drawMutex->unlock();
      }
    }
...
//    {
//        std::cout << "Logfilegro?e:  " << logs.log_size() << std::endl;
